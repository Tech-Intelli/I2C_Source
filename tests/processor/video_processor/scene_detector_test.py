...
@pytest.mark.parametrize("color1, color2", [((255, 0, 0), (0, 0, 255)), ((0, 255, 0), (0, 0, 255))])
def test_process_frame_with_color_changes(scene_detector, color1, color2):
    ...
    assert scene_detector.mean_diffs[0] > 0, f"Expected a positive mean diff, got {scene_detector.mean_diffs[0]}"
...
def test_reset_and_different_sizes(scene_detector):
    ...
    assert len(scene_detector.mean_diffs) == 1
    assert scene_detector.mean_diffs[0] > 0, f"Mean diff was not positive: {scene_detector.mean_diffs[0]}"
